...
from src.models.sequence.kernels.rtf_wrapper import RTF
from src.models.sequence.kernels.spacetime.companion import CompanionSSM
    ...
    def __init__(
        ...
        **layer_args,  # Arguments into inner layer (e.g. FFTConv)
        ...
        if(custom_layer is not None):
            if(custom_layer == 'rtf'):
                self.layer = RTF(d_model=d_model, transposed=False, **layer_args)
            elif(custom_layer == 'spacetime'):
                self.layer = CompanionSSM(model_dim=d_model, **layer_args)
            else:
