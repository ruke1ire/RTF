...
class HyenaOperator(nn.Module):
    ...
    def setup_projections(self, fused_bias_fc, inner_factor):
        ...
        self.out_proj = linear_cls(384, self.d_model) # for Low rank MIMO
        ...
        self.mimo_proj = linear_cls(self.d_model, 384)
    ...
    def forward(self, u, *args, **kwargs):
        ...
        *x, v = uc.split(self.d_model, dim=2)
        ...
        for o, x_i in enumerate(reversed(x[1:])):
            ...
            #v = torch.nn.functional.gelu(self.mimo_proj(v))
        ...
        y = torch.nn.functional.gelu(self.mimo_proj(self.norm(y)))
        y = self.out_proj(y)
    ...
    def forward2(self, u, *args, **kwargs):
        l = u.size(-2)
        l_filter = min(l, self.l_max)
        u = self.in_proj(u)
        u = rearrange(u, 'b l d -> b d l')
        ...
        *x, v = uc.split(self.d_model, dim=2)
        ...
        y = self.activation(rearrange(v * x[0], 'b h v z l -> b (z l) (h v)', z=self.num_blocks, h=self.num_heads))
        y = torch.nn.functional.gelu(self.mimo_proj(y))
        y = self.out_proj(y)
